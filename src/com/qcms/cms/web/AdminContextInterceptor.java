...
public class AdminContextInterceptor implements Interceptor {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 8767419079218032259L;
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		...
		if (user == null) {
			...
			// ba.setValue(BaseAction.RETURN_ACTION, returnAction);
			// ba.setValue(BaseAction.RETURN_ACTION_NAMESPACE,
			// returnActionNamespace);
		...
		/*
		 * boolean viewonly = user.getViewonlyAdmin(); // 没有访问权限，提示无权限。 if (auth
		 * && !user.isSuper() && !permistionPass(uri, user.getPerms(),
		 * viewonly)) { request.setAttribute(MESSAGE,
		 * MessageResolver.getMessage(request, "login.notPermission"));
